...
[Nix](https://nixos.org/nix/) is needed to run Daedalus in a `nix develop` shell.
...
3. Run `nix develop` with the correct argument to load a shell with all the correct versions of all the required dependencies for development.
...
1. Run `nix develop .#selfnode` from `daedalus`.
...
1. Run `nix develop` from `daedalus`.
2. Run `yarn dev` from the subsequent Nix shell.

##### Flight

1. Run `nix develop .#mainnet_flight` from `daedalus`.
2. Run `yarn dev` from the subsequent Nix shell.

##### Testnet

1. Run `nix develop .#testnet` from `daedalus`.
2. Run `yarn dev` from the subsequent Nix shell.
...
It's also possible to use a mock server locally by running the following command in a `nix develop` shell prior to starting Daedalus:
...
- NodeJS 16
- Python2
- `yarn global add windows-build-tools` (if this does not work extract daedalus\installers\windows-usb-libs.zip under daedalus\build folder)
...
Nix flakes are used to manage the version of upstream dependencies. The versions of these dependencies can be seen in `flake.nix` and `flake.lock`.

Dependencies are updated with the follow nix commands:
- Update cardano-wallet to the latest master: `nix flake lock --update-input cardano-wallet`
- Update cardano-wallet to a specific revision:
    1. Update the line in `flake.nix` to say: `inputs.cardano-wallet.url = "github:input-output-hk/cardano-wallet/91db88f9195de49d4fb4299c68fc3f6de09856ab";`
    2. Then run `nix flake lock --update-input cardano-wallet`
- Update cardano-node to a specific tag: it is a transitive dependency for us, so you have to edit `cabal.project` inside `cardano-wallet`, and then point our `flake.nix` to your new revision of `cardano-wallet`.

#### Notes

The `nix develop` shell also provides a script for updating `yarn.lock` file:

    nix develop .#fixYarnLock
...
If you use any 3rd party libraries which can't or won't be bundled by webpack, you must list them in your `source/main/webpack.config.js` and/or `source/renderer/webpack.config.js`：
...
From a Linux machine, run this:

    nix build .#daedalus.installer.mainnet

The installer’s `.bin` file will be in `./result/`.

#### Windows

From a Linux machine, run this:

    nix build .#daedalus-x86_64-windows.installer.mainnet

The installer’s `.exe` file will be in `./result/`.
